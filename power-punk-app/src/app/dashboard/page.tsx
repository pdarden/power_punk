...
const mockProjectData: Record<string, ProjectData> = {
  ...
  'walrus_2': {
    ...
    goalAmount: 500000,
    contributors: [
      { walletAddress: 'current_user', units: 20, totalAmountPaid: 100000, timestamp: '2024-01-02' },
    ],
    timeline: {
  ...
  const calculateTotalFunded = () => {
    return Object.values(projectData).reduce((total, data) => {
      const userContributions = data.contributors.filter(
        c => c.walletAddress === 'current_user' || c.walletAddress === '0x123...'
      );
      return total + userContributions.reduce((sum, c) => sum + c.totalAmountPaid, 0);
    }, 0);
  };
  ...
  const calculateTotalReferrals = () => {
    return Object.values(projectData).reduce((total, data) => {
      const userReferrals = data.referrals.filter(
        r => r.referrerWallet === 'current_user' || r.referrerWallet === '0x123...'
      );
      return total + userReferrals.reduce((sum, r) => sum + r.referredWallets.length, 0);
    }, 0);
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {fundedProjects.map((project) => {
                    ...
                    const userContribution = data?.contributors.find(
                      c => c.walletAddress === 'current_user'
                    );
                        ...
                        {userContribution && (
                          <div className="mt-2 p-2 bg-blue-50 rounded text-sm">
                            <p className="text-blue-700">
                              Your contribution: {formatCurrency(userContribution.totalAmountPaid)}
                            </p>
