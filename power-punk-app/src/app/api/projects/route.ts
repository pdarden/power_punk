...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectData, location, projectType } = body;

    // Store project data in Walrus
    ...
    const { data: campaign, error } = await supabase
      ...
      .insert({
        id: projectId,
        walrus_id: walrusId,
        owner_id: body.userId, // This should come from authenticated user
        status: 'active',
        location,
  ...
  try {
    ...
    const projectsWithData = await Promise.all(
      campaigns.map(async (campaign) => {
        try {
          const projectData = await walrusClient.getProjectData(campaign.walrus_id);
          return { campaign, projectData };
        } catch (error) {
          console.error(`Failed to fetch Walrus data for ${campaign.walrus_id}:`, error);
